/*
cron:27 0 * * * jdjr_tiantian_zhuan_dou.js
...
async function openMissionLink(cookie, mission, config) {
  if (!mission?.doLink) {
    return null;
  }
  ...
  const response = await got.get(mission.doLink, {
...
async function verifyMission(cookie, riskContext, config, mission) {
  if (canQueryMissionDetail(mission)) {
    ...
    return queryMissionDetail(cookie, riskContext, mission);
  }
...
async function handleMissionChannel(cookie, riskContext, config, prefix) {
  ...
  for (const mission of firstList) {
    ...
    if (mission.doLink) {
      const openResult = await openMissionLink(cookie, mission, config);
